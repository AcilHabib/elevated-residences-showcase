...
import { Building2, ArrowRight, Layers, Home } from "lucide-react";
...
export const Route = createFileRoute("/residence")({
  head: () => ({
    meta: [
      { title: "La Résidence — 6 blocs · P.B.A Béjaïa" },
      { name: "description", content: "Découvrez les 6 blocs résidentiels de la résidence P.B.A à Béjaïa." },
    ],
  }),
    ...
    <>
      <section className="relative pt-40 pb-20 overflow-hidden">
        <div className="absolute inset-0">
          <img src={aerialImg} alt="" className="h-full w-full object-cover opacity-30" />
          <div className="absolute inset-0 bg-gradient-to-b from-background/80 via-background/90 to-background" />
        </div>
        <div className="container-luxury relative">
          <Reveal>
            <p className="hairline-gold text-xs uppercase tracking-[0.25em] text-gold font-semibold">La Résidence</p>
            <h1 className="mt-4 font-display text-5xl md:text-7xl leading-tight max-w-4xl">
              Une architecture pensée comme <span className="italic text-gradient-gold">un art de vivre</span>
            </h1>
            <p className="mt-6 max-w-2xl text-lg text-muted-foreground leading-relaxed">
              Six blocs résidentiels, trente étages, des centaines de fenêtres ouvertes sur la Méditerranée.
              Découvrez l'interactif d'implantation des blocs et accédez aux plans détaillés.
            </p>
          </Reveal>
        </div>
      </section>
        ...
        <div className="container-luxury">
          <Reveal>
            <div className="relative aspect-[16/9] rounded-3xl overflow-hidden shadow-[var(--shadow-luxury)]">
              <img src={aerialImg} alt="Plan d'implantation" className="h-full w-full object-cover" />
              ...
              <Reveal key={b.id} delay={i * 0.05}>
                ...
                >
                  <div className="relative aspect-[4/3] overflow-hidden">
                    <img src={blockImg} alt={b.name} loading="lazy" className="h-full w-full object-cover transition-transform duration-[1200ms] group-hover:scale-110" />
                  ...
                  <div className="p-6">
                    ...
                    <div className="mt-2 flex items-center gap-4 text-xs uppercase tracking-[0.18em] text-muted-foreground">
                      <span className="flex items-center gap-1.5"><Layers className="h-3.5 w-3.5 text-gold" /> {b.floors} étages</span>
                      <span className="flex items-center gap-1.5"><Home className="h-3.5 w-3.5 text-gold" /> Plans PDF</span>
                    </div>
